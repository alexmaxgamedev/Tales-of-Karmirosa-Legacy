...
func _process(delta) -> void:
	...
	if (Input.is_action_just_pressed("run") and Input.is_action_pressed("move_forward")) or (Input.is_action_just_pressed("move_forward") and Input.is_action_pressed("run")):
		if self.is_on_floor() and direction != Vector3.ZERO:
